...
```
build/output/
├── rt950pro_firmware.elf    # ELF file (for debugging)
├── rt950pro_firmware.bin    # Binary file (for flashing)
├── rt950pro_firmware.hex    # Intel HEX format
├── rt950pro_firmware.map    # Memory map file
└── CMakeCache.txt
```
...
1. Open ST-Link Utility
2. Connect to target
3. File → Open File... → Select `rt950pro_firmware.hex` or `.bin`
